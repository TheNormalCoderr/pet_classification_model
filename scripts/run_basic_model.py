"""
Runner script for the basic CNN model experiment.

Usage (from project root):
    python scripts/run_basic_model.py

Outputs are saved to: outputs/basic_model/
  - basic_model_training_curves.png
...
from src.features import preprocess_basic
from src.model import build_basic_model, compile_model
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "basic_model"
...
model = compile_model(build_basic_model(num_classes=len(images_label)))
history = run_training(model, X_train, y_train, X_val, y_val, epochs=20, batch_size=32)
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "basic_model_training_curves.png")
plt.close()
...
results_df = save_results(
    ...
    test_loss,
    test_accuracy,
    OUTPUT_DIR / "training_validation_test_results.csv",
)
